...
async def idea(ctx):
    ...
    languages = ['JavaScript', 'C++', 'Python', 'Java', 'React']

    idea = f'Create a new {random.choice(topics)} that helps with {random.choice(areas)} in {random.choice(languages)}! :slight_smile:'
    await ctx.send(idea)
...
with open(".gitignore\BOT_TOKEN.txt", "r") as token_file:
